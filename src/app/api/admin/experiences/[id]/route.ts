...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  } catch (error) { console.error("[experiences PUT]", error); return NextResponse.json({ ok: false, error: "Error" }, { status: 500 }); }
}
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  } catch (error) { console.error("[experiences DELETE]", error); return NextResponse.json({ ok: false, error: "Error" }, { status: 500 }); }
}
